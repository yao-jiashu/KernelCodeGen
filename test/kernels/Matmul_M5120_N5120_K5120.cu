...
namespace Matmul_M5120_N5120_K5120 {
// grid dims:(40, 40, ), block dims:(16, 16, )
__global__ void kernel0(float* arg0, float* arg1, float* arg2) {
  ...
  auto vec0 = (reinterpret_cast<float4*>(&(arg0[((((threadIdx.y * 8) + (threadIdx.x / 2)) + (const2th * 128)) + (blockIdx.y * 128)) * 5120 + (((threadIdx.x % 2) * 4) + const7th) * 1 + 0]))[0]);
  (reinterpret_cast<float4*>(&(array2[(const2th * 4)]))[0]) = vec0;
  auto vec1 = (reinterpret_cast<float4*>(&(arg1[(((((threadIdx.y * 16) + threadIdx.x) / 32) + (const2th * 8)) + const8th) * 5120 + (((((threadIdx.y * 16) + threadIdx.x) % 32) * 4) + (blockIdx.x * 128)) * 1 + 0]))[0]);
  ...
  for (int iter3 = 0; iter3 < 5120; iter3 += 8) {
    if (((iter3 * -1) + 5104) >= 0 &&  true) {
      auto vec8 = (reinterpret_cast<float4*>(&(arg0[((((threadIdx.y * 8) + (threadIdx.x / 2)) + (const2th * 128)) + (blockIdx.y * 128)) * 5120 + (((threadIdx.x % 2) * 4) + (iter3 + 8)) * 1 + 0]))[0]);
      (reinterpret_cast<float4*>(&(array2[(const2th * 4)]))[0]) = vec8;
      auto vec9 = (reinterpret_cast<float4*>(&(arg1[(((((threadIdx.y * 16) + threadIdx.x) / 32) + (const2th * 8)) + (iter3 + 8)) * 5120 + (((((threadIdx.y * 16) + threadIdx.x) % 32) * 4) + (blockIdx.x * 128)) * 1 + 0]))[0]);
  ...
  for (int iter10 = 0; iter10 < 4; iter10 += 1) {
    ...
    (reinterpret_cast<float4*>(&(arg2[(((blockIdx.y * 128) + ((((((threadIdx.y * 16) + threadIdx.x) % 32) / 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) / 4) + ((const2th / 4) * 2)) * 8)) * 4)) + iter10) * 5120 + (((blockIdx.x * 128) + (((threadIdx.x % 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) % 4) + ((const2th / 4) * 4)) * 4)) * 4)) + const2th) * 1 + 0]))[0]) = vec20;
  }
  #pragma unroll
  for (int iter11 = 0; iter11 < 4; iter11 += 1) {
    auto vec21 = (reinterpret_cast<float4*>(&(array6[(const2th + iter11)][(const0th + const2th)]))[0]);
    (reinterpret_cast<float4*>(&(arg2[(((blockIdx.y * 128) + ((((((threadIdx.y * 16) + threadIdx.x) % 32) / 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) / 4) + ((const2th / 4) * 2)) * 8)) * 4)) + iter11) * 5120 + (((blockIdx.x * 128) + (((threadIdx.x % 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) % 4) + ((const0th / 4) * 4)) * 4)) * 4)) + const2th) * 1 + 0]))[0]) = vec21;
  }
  #pragma unroll
  for (int iter12 = 0; iter12 < 4; iter12 += 1) {
    auto vec22 = (reinterpret_cast<float4*>(&(array6[(const0th + iter12)][(const2th + const2th)]))[0]);
    (reinterpret_cast<float4*>(&(arg2[(((blockIdx.y * 128) + ((((((threadIdx.y * 16) + threadIdx.x) % 32) / 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) / 4) + ((const0th / 4) * 2)) * 8)) * 4)) + iter12) * 5120 + (((blockIdx.x * 128) + (((threadIdx.x % 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) % 4) + ((const2th / 4) * 4)) * 4)) * 4)) + const2th) * 1 + 0]))[0]) = vec22;
  }
  #pragma unroll
  for (int iter13 = 0; iter13 < 4; iter13 += 1) {
    auto vec23 = (reinterpret_cast<float4*>(&(array6[(const0th + iter13)][(const0th + const2th)]))[0]);
    (reinterpret_cast<float4*>(&(arg2[(((blockIdx.y * 128) + ((((((threadIdx.y * 16) + threadIdx.x) % 32) / 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) / 4) + ((const0th / 4) * 2)) * 8)) * 4)) + iter13) * 5120 + (((blockIdx.x * 128) + (((threadIdx.x % 4) + ((((((threadIdx.y * 16) + threadIdx.x) / 32) % 4) + ((const0th / 4) * 4)) * 4)) * 4)) + const2th) * 1 + 0]))[0]) = vec23;
  }
}
}
